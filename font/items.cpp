...
int Items::loadFromOtb(std::string file)
{
	...
	props.GET_UINT32(MajorVersion);
	props.GET_UINT32(MinorVersion);
	props.GET_UINT32(BuildNumber);
	...
	do{
		...
		//iType->forceuse = hasBitSet(___ForceUse, flags);
		...
		//iType->look = hasBitSet(___IgnoreLook, flags);
/*
		if (hasBitSet(___FullGround, flags))
			iType->group = ITEM_GROUP_GROUND;*/
		//iType->allowDistRead = hasBitSet(___IsAnimation, flags);
		...
		while(props.GET_UINT8(attrib)){
			...
			switch(attrib){
				...
				{
					lightBlock2 lb2;
					props.GET_UINT16(lb2.lightLevel);
					props.GET_UINT16(lb2.lightColor);
					iType->lightLevel = lb2.lightLevel;
					iType->lightColor = lb2.lightColor;
					break;
				}
				case ITEM_ATTR_StackOrder:
				{
					uint8_t stack;
					props.GET_UINT8(stack);
					iType->alwaysOnTopOrder = stack;

					break;
				}

				case ITEM_ATTR_ELEVATION:
				{
					uint16_t height;
					props.GET_UINT16(height);
					iType->height = height;
				}
	
				default:
				//skip unknown attributes
				props.SKIP_N(datalen);
				break;
			}
		}
				...
				if(readXMLInteger(itemNode, "id", intValue)){
					...
					while(itemAttributesNode){
						if(readXMLString(itemAttributesNode, "key", strValue)){
							...
							//-----------------------------------------
							//new attribute 
							else if (asLowerCaseString(strValue) == "docombatafterdecay"){
								if (readXMLInteger(itemAttributesNode, "value", intValue)){
									it.doCombatAfterDecay = (intValue == 1);
								}
							}
							//-----------------------------------------
