...
{	
	return getSkillValue(PLAYER_SKILL_PHYSICAL_ATTACK);
}

int32_t Player::getShieldDefense() const
{
...
int32_t Player::getWeaponDefense() const
{
	int32_t WeaponDefense = 0;
	const Item* shield = NULL;
	const Item* weapon = NULL;
	getShieldAndWeapon(shield, weapon);
	if (!weapon)
...
int32_t Player::getDefense() const
{
	...
	getShieldAndWeapon(shield, weapon);
	defenseSkill = getSkillValue(PLAYER_SKILL_PHYSICAL_DEFENSE);
...
{ 
	...
	return Creature::blockHit(attacker, combatType, blockType, data);
}
